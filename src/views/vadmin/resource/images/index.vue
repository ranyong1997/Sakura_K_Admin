...
<template>
    <ContentWrap>
        ...
        <Table v-model:limit="tableObject.limit" v-model:page="tableObject.page" :columns="columns"
            :data="tableObject.tableData" :loading="tableObject.loading" :selection="true" :size="tableSize" :border="true"
            :pagination="{
            ...
            <template #image_url="{ row, $index }">
                <div class="resource-image-name flex items-center">
                    <div>
                        <ElImage :src="`${row.image_url}?x-oss-process=image/resize,m_fixed,h_100`" :zoom-rate="1.2"
                            :preview-src-list="tableObject.tableData.map((item) => item.image_url)"
                            :preview-teleported="true" :initial-index="$index" style="height: 60px; display: block"
                            fit="cover" />
                    </div>
