...
<template>
    <Form :rules="rules" label-position="top" @register="register">
        <template #images>
            ...
            <ElUpload class="resource-image-uploader" action="#" :http-request="handleHttpRequest"
                v-model:file-list="fileList" :show-file-list="true" :multiple="true" :before-upload="beforeImageUpload"
                :on-success="handleUploadSuccess" :on-preview="handlePictureCardPreview" :on-exceed="handleExceedLimit"
                accept="image/jpeg,image/png" name="file" list-type="picture-card" :limit="maxLimit" :drag="true"
    ...
    <ElImageViewer v-if="dialogVisible" :z-index="9999" @close="handleCloseViewer"
        :url-list="fileList.map((item) => item.url as string)" :initial-index="imgIndex" />
</template>
